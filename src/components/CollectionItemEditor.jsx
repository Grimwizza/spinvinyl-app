    ...
    return (
        <div className="flex flex-col h-full bg-black">
            ...
            <div className="flex items-center justify-between px-4 py-4 bg-gray-950 border-b border-white/5"
                ...
                <button onClick={onCancel} className="text-sm font-semibold text-gray-400 hover:text-white transition-colors">Cancel</button>
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-1.5 -ml-1">
                            {[1, 2, 3, 4, 5].map(star => (
                                ...
                                >
                                    <Star size={28} strokeWidth={1.5} className={star <= rating ? 'fill-amber-400 text-amber-400 drop-shadow-md' : 'text-gray-600'} />
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <select value={folderId} onChange={e => setFolderId(e.target.value)} className="w-full appearance-none bg-white/[0.02] border border-white/[0.08] hover:border-white/20 hover:bg-white/[0.04] transition-colors rounded-xl px-4 py-3.5 text-[15px] text-white focus:outline-none focus:ring-2 ring-violet-500/50">
                                {folders.map(f => (
                    ...
                    <div className="space-y-6">
                        {fieldDefs.map(f => (
                            <div key={f.id} className={f.id === NOTES_ID ? 'pb-8' : undefined}>
                                <label className="block text-[13px] font-medium text-gray-400 mb-2">{f.name}</label>
                                {f.type === 'dropdown' ? (
                                    <div className="relative">
                                        <select
                                            value={fieldValues[f.id] ?? ''}
                                            onChange={e => setFieldValue(f.id, e.target.value)}
                                            className="w-full appearance-none bg-white/[0.02] border border-white/[0.08] hover:border-white/20 hover:bg-white/[0.04] transition-colors rounded-xl px-4 py-3.5 text-[15px] text-white focus:outline-none focus:ring-2 ring-violet-500/50"
                                        >
                                            {(f.options || []).map(c => <option className="bg-gray-900 text-white" key={c} value={c}>{c}</option>)}
                                        </select>
                                        <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
                                    ...
                                    <>
                                        <textarea
                                            ...
                                            className="w-full bg-white/[0.02] border border-white/[0.08] hover:border-white/20 transition-colors rounded-xl px-4 py-3.5 text-[15px] text-white placeholder-gray-600 focus:outline-none focus:ring-2 ring-violet-500/50 resize-none"
                                        ></textarea>
                                        <div className="text-right text-[11px] font-medium text-gray-500 mt-1.5">{(fieldValues[f.id] ?? '').length} / 255</div>
                                    </>
                                ) : (
                                    <input
                                        type="text"
                                        ...
                                        onChange={e => setFieldValue(f.id, e.target.value)}
                                        placeholder={`Enter ${f.name}`}
                                        className="w-full bg-white/[0.02] border border-white/[0.08] hover:border-white/20 transition-colors rounded-xl px-4 py-3.5 text-[15px] text-white placeholder-gray-600 focus:outline-none focus:ring-2 ring-violet-500/50"
                                    />
                                )}
                            </div>
                        ))}
            ...
            <div className="p-4 bg-gray-950 border-t border-white/5 flex-shrink-0 pb-safe">
                ...
                >
                    {saving ? <Loader2 size={18} className="animate-spin text-black" /> : 'Save changes'}
                </button>
            </div>
        </div>
    );
